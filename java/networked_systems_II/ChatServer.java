...
public class ChatServer {
     ...
     */
    private static HashSet<ChatMember> writers = new HashSet<ChatMember>();
    ...
    private static class Handler extends Thread {
        ...
        public void run() {
            try {
                logger.log(Level.INFO, "setting up socket input and output streams.");
                ...
                logger.log(Level.INFO, "Name: " + name
                        + " has been added to chat.");
                out.println("NAMEACCEPTED");

                writers.add(new ChatMember(name, out));
                ...
                while (true) {
                    ...
                    if (input.startsWith("BYE")) {
                        logger.log(Level.INFO, "Name: " + name + " has left the chat.");
                        out.println("TERMINATING");
                        return;
                    } else if (input.startsWith("DIR")) {
                        logger.log(Level.INFO, "Name: " + name + " sent a direct message.");
                        for (ChatMember member : writers) {
                        String[] arr = input.split(" ");
                        if (member.getName().equals(arr[1])) {
                            StringBuilder sb = new StringBuilder();
                            for (int  i = 2; i < arr.length; i++) {
                                sb.append(arr[i]);
                                sb.append(" ");
                            }
                            member.getWriter().println("MESSAGE " + "Direct message from " + name + ": " + sb);
                        }
                            }
                    } else if (input.startsWith("WHO")) {
                        logger.log(Level.INFO, "Name: " + name + " requested a member lookup.");
                        out.println("MESSAGE " + "Current members:");
                        for (ChatMember member : writers) {
                            out.println("MESSAGE " + member.getName());
                        }
                    } else {
                        //message everyone 
                        for (ChatMember member : writers) {
                            member.getWriter().println("MESSAGE " + name
                                    + ": " + input);
                        }
                        logger.log(Level.INFO, "Name: " + name + " has sent a message: " + input);
                    }
                }
            } catch (IOException e) {
